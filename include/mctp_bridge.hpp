...
class MCTPBridge : public MCTPEndpoint {
  public:
	MCTPBridge(boost::asio::io_context &ioc, std::shared_ptr<object_server> &objServer);
	MCTPBridge()  = delete;
	~MCTPBridge() = default;
  ...
  protected:
	...
	bool getMctpVersionSupportCtrlCmd(boost::asio::yield_context &yield,
									  const std::vector<uint8_t> &bindingPrivate,
									  const mctp_eid_t			  destEid,
									  uint8_t					  msgTypeNo,
									  MctpVersionSupportCtrlResp *mctpVersionSupportCtrlResp);
	void getVendorDefinedMessageTypes(boost::asio::yield_context  yield,
									  const std::vector<uint8_t> &bindingPrivate,
									  mctp_eid_t				  destEid,
									  EndpointProperties		 &epProperties);
	// vendor PCI ID Function
	bool getPCIVDMessageSupportCtrlCmd(boost::asio::yield_context &yield,
									   const std::vector<uint8_t> &bindingPrivate,
									   const mctp_eid_t			   destEid,
									   std::vector<uint16_t>	  &vendorSetIdList,
									   std::string				  &venformat);
	bool getRoutingTableCtrlCmd(boost::asio::yield_context &yield,
								...
								std::vector<uint8_t>	   &resp);
	//   private:
	std::optional<mctp_eid_t> busOwnerRegisterEndpoint(boost::asio::yield_context &yield,
													   const std::vector<uint8_t> &bindingPrivate,
													   mctp_eid_t				   eid);
	void					  sendRoutingTableEntriesToBridge(const mctp_eid_t			  bridge,
															  const std::vector<uint8_t> &bindingPrivate);
	void sendNewRoutingTableEntryToAllBridges(const mctpd::RoutingTable::Entry &entry);

  private:
	void logUnsupportedMCTPVersion(const std::vector<struct MCTPVersionFields> versionsData,
								   const mctp_eid_t							   eid);
	void
	sendRoutingTableEntries(const std::vector<mctpd::RoutingTable::Entry::MCTPLibData> &entries,
