...
class MCTPDevice : public MCTPDBusInterfaces {
  public:
	MCTPDevice(boost::asio::io_context &ioc, std::shared_ptr<object_server> &objServer);
	MCTPDevice() = delete;
	~MCTPDevice();
  ...
  protected:
	...
	std::unordered_map<uint8_t, version_entry> versionNumbersForUpperLayerResponder;
	// <eid, uuid>
	std::unordered_map<mctp_eid_t, std::string> uuidTable;
	struct mctp								   *mctp = nullptr;

	virtual std::optional<std::string> getLocationCode(const std::vector<uint8_t> &bindingPrivate);
	virtual void					   updateRoutingTableEntry(mctpd::RoutingTable::Entry  entry,
															   const std::vector<uint8_t> &privateData);
	virtual std::optional<std::vector<uint8_t>> getBindingPrivateData(uint8_t dstEid);
	...
	std::optional<mctp_eid_t> getEIDForReregistration(const std::string &destUUID);
  ...
  private:
	...
	//  Callback>
	std::vector<std::tuple<PacketState,
						   uint8_t,
						   unsigned int,
						   mctp_eid_t,
						   std::vector<uint8_t>,
	...
	bool sendMctpCtrlMessage(mctp_eid_t			  destEid,
							 std::vector<uint8_t> req,
							 bool				  tagOwner,
							 uint8_t			  msgTag,
							 std::vector<uint8_t> bindingPrivate);
	void pushToCtrlTxQueue(PacketState				   pktState,
						   const mctp_eid_t			   destEid,
						   const std::vector<uint8_t> &bindingPrivate,
						   const std::vector<uint8_t> &req,
