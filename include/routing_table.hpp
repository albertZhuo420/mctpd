...
{
	return type == EndPointType::BridgeAndEndPoints || type == EndPointType::BridgeOnly ||
...
class RoutingTable {
  public:
	struct Entry {
		...
		{
			EndPointType type = static_cast<EndPointType>(routeEntry.routing_info.entry_type);
			return type == EndPointType::BridgeAndEndPoints || type == EndPointType::BridgeOnly ||
				   type == EndPointType::BridgeWithoutEndpoint;
		}
	...
	inline bool		   contains(mctp_eid_t eid) { return entries.count(eid) == 1; }
