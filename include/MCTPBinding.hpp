...
class MctpBinding : public MCTPBridge {
  ...
  protected:
	...
	virtual void addUnknownEIDToDeviceTable(const mctp_eid_t eid, void *bindingPrivate);

	void initializeMctp();
	bool registerUpperLayerResponder(uint8_t typeNo, std::vector<uint8_t> &list);
	...
	std::optional<mctp_eid_t> registerEndpoint(
		boost::asio::yield_context	 &yield,
		const std::vector<uint8_t>	 &bindingPrivate,
		mctp_eid_t					  eid,
		mctp_server::BindingModeTypes bindingMode = mctp_server::BindingModeTypes::Endpoint);
	// MCTP Callbacks
	static void rxMessage(uint8_t srcEid,
						  void	 *data,
						  void	 *msg,
						  size_t  len,
						  bool	  tagOwner,
	...
	static void onRawMessage(void *data, void *msg, size_t len, void *msgBindingPrivate);
	static void handleMCTPControlRequests(uint8_t srcEid,
										  void	 *data,
										  void	 *msg,
										  size_t  len,
										  bool	  tagOwner,
	...
	void registerProperty(
		...
		sdbusplus::asio::PropertyPermission access = sdbusplus::asio::PropertyPermission::readOnly)
	{
		if (ifc->register_property(name, property, access) != true) {
			throw std::invalid_argument(name);
		}
	}
	bool setMediumId(uint8_t value, mctp_server::MctpPhysicalMediumIdentifiers &mediumId);
