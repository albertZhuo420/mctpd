...
using endpointInterfaceMap = std::unordered_map<mctp_eid_t, std::shared_ptr<dbus_interface>>;
...
class MCTPDBusInterfaces {
  public:
	...
	inline const std::string &getDbusName() const { return serviceName; }
	// Set own service name
	inline void setDbusName(const std::string &sName) { serviceName = sName; }
  ...
  protected:
	...
	virtual void populateDeviceProperties(const mctp_eid_t			  eid,
