...
class SMBusBinding : public MctpBinding {
  public:
	...
	std::optional<std::vector<uint8_t>> getBindingPrivateData(uint8_t dstEid) override;
	bool								handleGetEndpointId(mctp_eid_t			  destEid,
															void				 *bindingPrivate,
															std::vector<uint8_t> &request,
															std::vector<uint8_t> &response) override;
	bool								handleSetEndpointId(mctp_eid_t			  destEid,
															void				 *bindingPrivate,
															std::vector<uint8_t> &request,
															std::vector<uint8_t> &response) override;
	bool								handleGetVersionSupport(mctp_eid_t			  destEid,
																void				 *bindingPrivate,
																std::vector<uint8_t> &request,
																std::vector<uint8_t> &response) override;
	bool								handleGetMsgTypeSupport(mctp_eid_t			  destEid,
																void				 *bindingPrivate,
																std::vector<uint8_t> &request,
																std::vector<uint8_t> &response) override;
	bool								handleGetVdmSupport(mctp_eid_t			  endpointEid,
															void				 *bindingPrivate,
															std::vector<uint8_t> &request,
															std::vector<uint8_t> &response) override;
	void addUnknownEIDToDeviceTable(const mctp_eid_t eid, void *bindingPrivate) override;
	...
	void updateRoutingTableEntry(mctpd::RoutingTable::Entry	 entry,
								 const std::vector<uint8_t> &privateData) override;

  private:
	using DeviceTableEntry_t =
		std::pair<mctp_eid_t /*eid*/, struct mctp_smbus_pkt_private /*binding prv data*/>;
	std::string SMBusInit();
	void		readResponse();
	void		initEndpointDiscovery(boost::asio::yield_context &yield);
	bool		reserveBandwidth(const mctp_eid_t eid, const uint16_t timeout) override;
	void		startTimerAndReleaseBW(const uint16_t interval, const mctp_smbus_pkt_private prvt);
	...
	void			   scanPort(const int scanFd, std::set<std::pair<int, uint8_t>> &deviceMap);
	void			   scanMuxBus(std::set<std::pair<int, uint8_t>> &deviceMap);
	mctp_eid_t		   getEIDFromDeviceTable(const std::vector<uint8_t> &bindingPrivate);
	...
	mctp_server::BindingModeTypes getBindingMode(const DeviceTableEntry_t &deviceTableEntry);
	bool						  isDeviceEntryPresent(const DeviceTableEntry_t				 &deviceEntry,
	...
	void   processRoutingTableChanges(const std::vector<DeviceTableEntry_t> &newTable,
