...
using pcie_binding = sdbusplus::xyz::openbmc_project::MCTP::Binding::server::PCIe;
...
class PCIeBinding :
  ...
  protected:
	bool handlePrepareForEndpointDiscovery(mctp_eid_t			 destEid,
										   void					*bindingPrivate,
										   std::vector<uint8_t> &request,
										   std::vector<uint8_t> &response) override;
	bool handleEndpointDiscovery(mctp_eid_t			   destEid,
								 void				  *bindingPrivate,
								 std::vector<uint8_t> &request,
								 std::vector<uint8_t> &response) override;
  ...
  private:
	...
	void	 processRoutingTableChanges(const std::vector<routingTableEntry_t> &newTable,
	...
	void	 readRoutingTable(std::vector<routingTableEntry_t> &rt,
							  ...
							  uint16_t							physAddr,
							  long								entryIndex = 0);
	uint16_t getRoutingEntryPhysAddr(const std::vector<uint8_t> &getRoutingTableEntryResp,
									 size_t						 entryOffset);
	...
	bool	 allBridgesCalled(const std::vector<routingTableEntry_t> &rt,
							  const std::vector<calledBridgeEntry_t> &calledBridges);
	...
	mctp_server::BindingModeTypes getBindingMode(const routingTableEntry_t &routingEntry);
