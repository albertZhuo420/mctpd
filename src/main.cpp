...
std::shared_ptr<MctpBinding> getBindingPtr(const Configuration &configuration,
										   ...
										   boost::asio::io_context		  &ioc)
{
	...
	if (auto smbusConfig = dynamic_cast<const SMBusConfiguration *>(&configuration)) {
	...
	else if (auto pcieConfig = dynamic_cast<const PcieConfiguration *>(&configuration)) {
		return std::make_shared<PCIeBinding>(conn,
											 objectServer,
											 mctpBaseObj,
											 *pcieConfig,
											 ioc,
...
int main(int argc, char *argv[])
{
	...
	std::optional<std::pair<std::string, std::unique_ptr<Configuration>>> mctpdConfigurationPair;

	app.add_option("-b,--binding", binding, "MCTP Physical Binding. Supported: -b smbus, -b pcie")
		->required();
	...
	signals.async_wait([&ioc, &bindingPtr](const boost::system::error_code &, const int &) {
	...
	try {
		...
		phosphor::logging::log<phosphor::logging::level::ERR>("Invalid configuration; exiting");
		return -1;
	}
	...
	if (!bindingPtr) {
		phosphor::logging::log<phosphor::logging::level::ERR>("Unable to create MCTP binding");
		return -1;
	}
