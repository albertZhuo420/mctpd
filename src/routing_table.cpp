...
EndPointType convertToEndpointType(const std::string &endpointType)
{
	...
	if (endpointType == "xyz.openbmc_project.MCTP.Base.BindingModeTypes.Endpoint") {
		type = EndPointType::EndPoint;
	}
	else if (endpointType == "xyz.openbmc_project.MCTP.Base.BindingModeTypes.Bridge") {
...
PhysicalMediumIdentifier
convertToPhysicalMediumIdentifier(mctp_server::MctpPhysicalMediumIdentifiers medium)
{
	...
	case mctp_server::MctpPhysicalMediumIdentifiers::Smbus3OrI2c400khzCompatible: {
...
const std::string &RoutingTable::getServiceName(const mctp_eid_t eid) const
{
	...
	throw std::out_of_range(std::string("EID not found. ") + std::to_string(eid));
}
...
bool RoutingTable::updateEntry(const mctp_eid_t eid, RoutingTable::Entry entry)
{
	...
	for (auto &[i, e] : table) {
		std::stringstream ss;
		ss << "Entry " << static_cast<int>(e.routeEntry.routing_info.starting_eid) << " Type "
		   << static_cast<int>(e.routeEntry.routing_info.entry_type) << " Medium "
		   << static_cast<int>(e.routeEntry.routing_info.phys_media_type_id) << " Medium ID "
		   << static_cast<int>(e.routeEntry.routing_info.phys_transport_binding_id) << " Address ";
		for (int addr : e.routeEntry.phys_address) { ss << addr << ' '; }
