...
using MctpPropertiesVariantType = std::variant<uint16_t,
											   int16_t,
											   int32_t,
											   uint32_t,
											   bool,
...
MCTPServiceScanner::MCTPServiceScanner(std::shared_ptr<sdbusplus::asio::connection> &conn) :
  connection(conn)
{
	if (!connection) {
...
static mctp_eid_t getEIDFromPath(const sdbusplus::message::object_path &object_path)
{
	try {
		...
		throw std::runtime_error(std::string("Error getting eid from ") + object_path.str + ". " +
								 e.what());
	}
}
						  ...
						  const std::string			  &service)
{
	...
	pid = connection.yield_method_call<uint32_t>(yield,
												 ec,
												 ...
												 service.c_str());
	if (ec) {
		std::string errMsg = std::string("MCTPServiceScanner. Error getting pid for ") + service +
							 ". " + ec.message();
										  ...
										  const std::string			&serviceName)
{
	...
	if (it == cachedServices.end()) {
		...
		serviceDetails.bindingID	   = readPropertyValue<std::string>(yield,
																	*connection,
																	serviceName,
																	baseObj,
																	baseIntf,
																	"BindingID");
		serviceDetails.bindingMediumID = readPropertyValue<std::string>(yield,
																		*connection,
																		serviceName,
																		baseObj,
																		baseIntf,
																		"BindingMediumID");
		serviceDetails.bindingMode	   = readPropertyValue<std::string>(yield,
																	*connection,
																	serviceName,
																	baseObj,
																	baseIntf,
...
{
	try {
		if (isSelfProcess(*connection, yield, serviceName) || !isAllowedBus(serviceName, yield)) {
			return;
		}
		...
		ObjectTree				  values;
		boost::system::error_code ec;
		values = connection->yield_method_call<ObjectTree>(yield,
														   ec,
														   serviceName,
		...
		if (ec) {
			std::string errMsg =
				std::string("MCTPServiceScanner. Error getting objects. ") + ec.message();
		...
		phosphor::logging::log<phosphor::logging::level::INFO>(("MCTPServiceScanner found " +
																std::to_string(values.size()) +
																" EIDs in " + serviceName)
																   .c_str());
		for (const auto &[objectPath, interfaces] : values) {
			if (interfaces.find("xyz.openbmc_project.MCTP.Endpoint") == interfaces.end()) {
				phosphor::logging::log<phosphor::logging::level::WARNING>(
					(static_cast<std::string>(objectPath) + " is not an MCTP endpoint").c_str());
				return;
			}

			EndPoint ep;
			ep.eid			= getEIDFromPath(objectPath);
			ep.endpointType = readPropertyValue<std::string>(yield,
															 *connection,
															 serviceName,
															 objectPath,
...
std::vector<std::string> MCTPServiceScanner::getMCTPServices(boost::asio::yield_context yield)
{
	std::vector<std::string>  requiredInterfaces = {"xyz.openbmc_project.MCTP.Base"};
	boost::system::error_code ec;
	DictType<std::string, std::vector<std::string>> services;

	// Get all available MCTP service names
	services =
		connection->yield_method_call<decltype(services)>(yield,
														  ec,
	...
	if (ec) {
		std::string errMsg = std::string("EidScanner: Error getting mctp services or no other "
...
void MCTPServiceScanner::scan()
{
	auto scanTask = [this](boost::asio::yield_context yield) {
		...
		phosphor::logging::log<phosphor::logging::level::INFO>("Scanning for other MCTP services");

		try {
			this->signalMatches.clear();
			auto onHotplugHandler =
				std::bind(&MCTPServiceScanner::onHotPluggedEid, this, std::placeholders::_1);
			this->signalMatches.emplace_back(
				registerSignalHandler(*connection,
									  onHotplugHandler,
									  "org.freedesktop.DBus.ObjectManager",
									  "InterfacesAdded",
									  "/xyz/openbmc_project/mctp"));

			auto onDeviceRemoveHandler =
				std::bind(&MCTPServiceScanner::onEidRemoved, this, std::placeholders::_1);
			this->signalMatches.emplace_back(
				registerSignalHandler(*connection,
									  onDeviceRemoveHandler,
									  "org.freedesktop.DBus.ObjectManager",
									  "InterfacesRemoved",
									  "/xyz/openbmc_project/mctp"));
			...
			for (const auto &service : mctpServices) { scanForEIDs(service, yield); }
		}
		catch (const std::exception &e) {
			phosphor::logging::log<phosphor::logging::level::WARNING>(
...
bool MCTPServiceScanner::isAllowedBus(const std::string &bus, boost::asio::yield_context yield)
{
	...
	for (const auto &service : allowedDestBuses) {
		...
		std::string				  uniqueName =
			connection->yield_method_call<std::string>(yield,
													   ec,
													   "org.freedesktop.DBus",
													   "/org/freedesktop/DBus",
													   "org.freedesktop.DBus",
		...
		if (ec) {
			std::string errMsg =
				std::string("GetUniqueName unsuccesful for ") + service + ". " + ec.message();
			phosphor::logging::log<phosphor::logging::level::WARNING>(errMsg.c_str());
			continue;
		}

		phosphor::logging::log<phosphor::logging::level::INFO>(
			("Unique name of " + service + " is " + uniqueName + ". Target " + bus).c_str());
...
void MCTPServiceScanner::onHotPluggedEid(sdbusplus::message::message &message)
{
	...
	DictType<std::string, DictType<std::string, MctpPropertiesVariantType>> values;
	...
	try {
		...
		ep.endpointType = std::get<std::string>(endpointIntf->second.at("Mode"));
		boost::asio::spawn(
			...
			[this, ep, message](boost::asio::yield_context yield) mutable {
				try {
					...
					if (!isAllowedBus(serviceName, yield)) {
						phosphor::logging::log<phosphor::logging::level::INFO>(
							(serviceName + " is not in allowed service list").c_str());
						return;
					}
					ep.service = this->getMctpServiceDetails(yield, message.get_sender());
					this->onNewEid(ep, true);
				}
				catch (const std::exception &e) {
					phosphor::logging::log<phosphor::logging::level::ERR>(
						(std::string("onHotPluggedEid. error in spawn task. ") + e.what())
							.c_str());
				}
			});
	}
...
void MCTPServiceScanner::onEidRemoved(sdbusplus::message::message &message)
{
	...
	try {
		...
		auto baseIntf =
			std::find(interfaces.begin(), interfaces.end(), "xyz.openbmc_project.MCTP.Base");
		if (baseIntf != interfaces.end()) {
			std::string serviceName = message.get_sender();
			// An MCTP service is going down.
			if (this->cachedServices.erase(serviceName) > 0) {
				phosphor::logging::log<phosphor::logging::level::INFO>(
					(message.get_sender() + std::string(" removed from cached services")).c_str());
			}
		...
		auto endpointIntf =
			std::find(interfaces.begin(), interfaces.end(), "xyz.openbmc_project.MCTP.Endpoint");
			...
			[this, ep, message](boost::asio::yield_context yield) mutable {
				try {
					...
					if (this->cachedServices.count(message.get_sender()) > 0) {
						ep.service = getMctpServiceDetails(yield, message.get_sender());
					}
					else {
					...
					phosphor::logging::log<phosphor::logging::level::ERR>(
						(std::string("onEidRemoved. error in spawn task. ") + e.what()).c_str());
				}
			});
	}
	catch (const std::exception &e) {
