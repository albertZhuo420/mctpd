...
PCIeBinding::PCIeBinding(std::shared_ptr<sdbusplus::asio::connection> conn,
  ...
  MctpBinding(conn, objServer, objPath, conf, ioc, mctp_server::BindingTypes::MctpOverPcieVdm),
  hw {std::move(hwParam)}, hwMonitor {std::move(hwMonitorParam)},
  getRoutingInterval(conf.getRoutingInterval), getRoutingTableTimer(ioc, getRoutingInterval)
{
	...
	try {
		...
		registerProperty(pcieInterface,
						 "DiscoveredFlag",
		...
		if (bindingModeType != mctp_server::BindingModeTypes::BusOwner) {
			getRoutingTableTimer.async_wait(std::bind(&PCIeBinding::updateRoutingTable, this));
		}
	}
...
void PCIeBinding::endpointDiscoveryFlow()
{
	...
	std::vector<uint8_t> prvData   = std::vector<uint8_t>(pktPrvPtr, pktPrvPtr + sizeof pktPrv);
	...
	boost::asio::spawn(io, [prvData, this](boost::asio::yield_context yield) {
		if (!discoveryNotifyCtrlCmd(yield, prvData, MCTP_EID_NULL)) {
			phosphor::logging::log<phosphor::logging::level::ERR>("Discovery Notify failed");
		}
	});
}

mctp_server::BindingModeTypes PCIeBinding::getBindingMode(const routingTableEntry_t &routingEntry)
{
	...
	case MCTP_ROUTING_ENTRY_BRIDGE: return mctp_server::BindingModeTypes::Bridge;
	case MCTP_ROUTING_ENTRY_ENDPOINT:
	case MCTP_ROUTING_ENTRY_ENDPOINTS:
	default: return mctp_server::BindingModeTypes::Endpoint;
	}
}

uint16_t PCIeBinding::getRoutingEntryPhysAddr(const std::vector<uint8_t> &getRoutingTableEntryResp,
											  size_t					  entryOffset)
{
	return be16toh(static_cast<uint16_t>(
		static_cast<uint16_t>(getRoutingTableEntryResp[entryOffset]) |
		(static_cast<uint16_t>(getRoutingTableEntryResp[entryOffset + 1]) << 8)));
}

bool PCIeBinding::isEntryInRoutingTable(get_routing_table_entry				   *routingEntry,
										const std::vector<routingTableEntry_t> &rt)
{
	return std::find_if(rt.begin(), rt.end(), [&routingEntry](const auto &entry) {
			   const auto &[eid, endpointBdf, entryType] = entry;
			   return routingEntry->starting_eid == eid;
		   }) != rt.end();
}

bool PCIeBinding::isActiveEntryBehindBridge(get_routing_table_entry				   *routingEntry,
											const std::vector<routingTableEntry_t> &rt)
{
	return !isEntryInRoutingTable(routingEntry, rt) && routingEntry->eid_range_size == 1 &&
		   routingEntry->phys_transport_binding_id == MCTP_BINDING_PCIE;
}

bool PCIeBinding::isEndOfGetRoutingTableResp(uint8_t entryHandle, uint8_t &responseCount)
{
...
{
	return GET_ROUTING_ENTRY_TYPE(std::get<2>(routingEntry)) == MCTP_ROUTING_ENTRY_BRIDGE ||
		   GET_ROUTING_ENTRY_TYPE(std::get<2>(routingEntry)) ==
			   MCTP_ROUTING_ENTRY_BRIDGE_AND_ENDPOINTS;
}

bool PCIeBinding::allBridgesCalled(const std::vector<routingTableEntry_t> &rt,
...
bool PCIeBinding::isBridgeCalled(const routingTableEntry_t				&routingEntry,
								 const std::vector<calledBridgeEntry_t> &calledBridges)
{
	return std::find_if(calledBridges.begin(),
						calledBridges.end(),
...
void PCIeBinding::readRoutingTable(std::vector<routingTableEntry_t> &rt,
								   ...
								   long								 entryIndex)
{
	...
	while (!isEndOfGetRoutingTableResp(entryHandle, responseCount)) {
		...
		if (!getRoutingTableCtrlCmd(yield, prvData, eid, entryHandle, getRoutingTableEntryResp)) {
			phosphor::logging::log<phosphor::logging::level::ERR>("Get Routing Table failed");
			return;
		}

		auto routingTableHdr =
			reinterpret_cast<mctp_ctrl_resp_get_routing_table *>(getRoutingTableEntryResp.data());
		...
		for (uint8_t i = 0; i < routingTableHdr->number_of_entries; i++) {
			...
			if (routingTableEntry->phys_transport_binding_id != MCTP_BINDING_PCIE) {
			...
			if (eid == busOwnerEid && GET_ROUTING_ENTRY_TYPE(routingTableEntry->entry_type) ==
										  MCTP_ROUTING_ENTRY_BRIDGE_AND_ENDPOINTS) {
			...
			else if (eid != busOwnerEid && isActiveEntryBehindBridge(routingTableEntry, rt)) {
...
void PCIeBinding::processBridgeEntries(std::vector<routingTableEntry_t> &rt,
									   std::vector<calledBridgeEntry_t> &calledBridges,
									   boost::asio::yield_context		&yield)
{
	...
	for (auto entry = rt.begin(); entry != rt.end(); entry++) {
		...
		uint8_t				*pktPrvPtr = reinterpret_cast<uint8_t *>(&pktPrv);
		std::vector<uint8_t> prvData =
			std::vector<uint8_t>(pktPrvPtr, pktPrvPtr + sizeof(mctp_astpcie_pkt_private));

		long entryIndex = std::distance(rt.begin(), entry);
		readRoutingTable(rtCopy,
						 calledBridges,
						 prvData,
						 yield,
						 std::get<0>(*entry),
...
void PCIeBinding::updateRoutingTable()
{
	...
	boost::asio::spawn(io, [prvData, this](boost::asio::yield_context yield) {
		...
		readRoutingTable(routingTableTmp, calledBridges, prvData, yield, busOwnerEid, busOwnerBdf);
...
void PCIeBinding::populateDeviceProperties(const mctp_eid_t			   eid,
										   const std::vector<uint8_t> &bindingPrivate)
{
	auto pcieBindingPvt =
		reinterpret_cast<const mctp_astpcie_pkt_private *>(bindingPrivate.data());
	...
	pcieIntf = objectServer->add_interface(mctpEpObj,
										   "xyz.openbmc_project.Inventory.Decorator.PCIDevice");
	pcieIntf->register_property("Bus", hw::bdf::getBus(pcieBindingPvt->remote_id));
	pcieIntf->register_property("Device", hw::bdf::getDevice(pcieBindingPvt->remote_id));
	pcieIntf->register_property("Function", hw::bdf::getFunction(pcieBindingPvt->remote_id));
	pcieIntf->initialize();
 ...
 */
void PCIeBinding::processRoutingTableChanges(const std::vector<routingTableEntry_t> &newTable,
											 boost::asio::yield_context				&yield,
											 const std::vector<uint8_t>				&prvData)
{
	...
	for (auto &routingEntry : routingTable) {
		if (find(newTable.begin(), newTable.end(), routingEntry) == newTable.end()) {
	...
	for (auto &routingEntry : newTable) {
		if (find(routingTable.begin(), routingTable.end(), routingEntry) == routingTable.end()) {
			...
			registerEndpoint(yield, prvDataCopy, remoteEid, getBindingMode(routingEntry));
			...
			functionHex << std::hex << hw::bdf::getFunction(pciePrivate->remote_id);

			std::string bus(busHex.str()), device(deviceHex.str()), function(functionHex.str());

			phosphor::logging::log<phosphor::logging::level::INFO>(
				("PCIe device " + bus + ":" + device + "." + function + " registered at EID " +
...
bool PCIeBinding::setDriverEndpointMap(const std::vector<routingTableEntry_t> &newTable)
{
...
{
	auto pciePrivate = reinterpret_cast<const mctp_astpcie_pkt_private *>(bindingPrivate);
...
bool PCIeBinding::handlePrepareForEndpointDiscovery(mctp_eid_t,
													void *bindingPrivate,
													std::vector<uint8_t> &,
													std::vector<uint8_t> &response)
{
	...
	auto pciePrivate = reinterpret_cast<mctp_astpcie_pkt_private *>(bindingPrivate);
	...
	auto resp = reinterpret_cast<mctp_ctrl_resp_prepare_discovery *>(response.data());
...
bool PCIeBinding::handleEndpointDiscovery(mctp_eid_t,
										  void *bindingPrivate,
										  std::vector<uint8_t> &,
										  std::vector<uint8_t> &response)
{
	...
	auto resp = reinterpret_cast<mctp_ctrl_resp_endpoint_discovery *>(response.data());
...
bool PCIeBinding::handleGetEndpointId(mctp_eid_t			destEid,
									  void				   *bindingPrivate,
...
bool PCIeBinding::handleSetEndpointId(mctp_eid_t			destEid,
									  void				   *bindingPrivate,
									  std::vector<uint8_t> &request,
									  std::vector<uint8_t> &response)
{
...
bool PCIeBinding::handleGetVdmSupport(mctp_eid_t			destEid,
									  void				   *bindingPrivate,
									  std::vector<uint8_t> &request,
									  std::vector<uint8_t> &response)
{
...
void PCIeBinding::initializeBinding()
{
	...
	if (binding == nullptr) {
		phosphor::logging::log<phosphor::logging::level::ERR>("Error in MCTP binding init");
	...
	if (status < 0) {
		...
		throw std::system_error(std::make_error_code(static_cast<std::errc>(-status)));
	}
...
std::optional<std::vector<uint8_t>> PCIeBinding::getBindingPrivateData(uint8_t dstEid)
{
	mctp_astpcie_pkt_private pktPrv = {};

	pktPrv.routing = PCIE_ROUTE_BY_ID;
	auto it = find_if(routingTable.begin(), routingTable.end(), [&dstEid](const auto &entry) {
		const auto &[eid, endpointBdf, entryType] = entry;
		return eid == dstEid;
	});
	if (it == routingTable.end()) {
		phosphor::logging::log<phosphor::logging::level::INFO>("Eid not found in routing table");
		return std::nullopt;
	}
...
{
	discoveredFlag = flag;
	pcieInterface->set_property("DiscoveredFlag",
								pcie_binding::convertDiscoveryFlagsToString(flag));
