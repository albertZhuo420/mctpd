...
using smbus_server = sdbusplus::xyz::openbmc_project::MCTP::Binding::server::SMBus;
...
void SMBusBinding::scanPort(const int scanFd, std::set<std::pair<int, uint8_t>> &deviceMap)
{
	if (scanFd < 0) {
		phosphor::logging::log<phosphor::logging::level::ERR>("Invalid I2C port fd");
		return;
	}
	...
	for (uint8_t it : supportedEndpointSlaveAddress) {
		...
		if (scanFd != outFd && rootDeviceMap.count(std::make_pair(outFd, it)) != 0) {
...
static bool findFiles(const fs::path		   &dirPath,
...
static bool getRootBus(const std::string &muxBus, std::string &rootBus)
{
	...
	auto path =
		fs::read_symlink(fs::path("/sys/bus/i2c/devices/i2c-" + muxBus + "/mux_device"), ec);
	if (ec) {
		return false;
	}
...
{
	return is_symlink(fs::path("/sys/bus/i2c/devices/i2c-" + bus + "/mux_device"));
}
...
std::optional<std::vector<uint8_t>> SMBusBinding::getBindingPrivateData(uint8_t dstEid)
{
...
bool SMBusBinding::reserveBandwidth(const mctp_eid_t eid, const uint16_t timeout)
{
	if (rsvBWActive && eid != reservedEID) {
		phosphor::logging::log<phosphor::logging::level::WARNING>(
			(("reserveBandwidth is not allowed for EID: " + std::to_string(eid) +
			  ". It is active for EID: ") +
...
bool SMBusBinding::releaseBandwidth(const mctp_eid_t eid)
{
	if (!rsvBWActive || eid != reservedEID) {
		phosphor::logging::log<phosphor::logging::level::ERR>(
			(("reserveBandwidth is not active for EID: ") + std::to_string(eid)).c_str());
		return false;
	}
										  ...
										  const mctp_smbus_pkt_private prvt)
{
	...
	// cancelled.
	ret = reserveBWTimer.expires_after(std::chrono::milliseconds(interval * 1000));
	reserveBWTimer.async_wait([this, prvt](const boost::system::error_code &ec) {
...
SMBusBinding::SMBusBinding(
  ...
  MctpBinding(conn, objServer, objPath, conf, ioc, mctp_server::BindingTypes::MctpOverSmbus),
  smbusReceiverFd(ioc), reserveBWTimer(ioc), scanTimer(ioc), addRootDevices(true),
  muxMonitor {std::move(i2cMuxMonitor)}, refreshMuxTimer(ioc)
{
	...
	try {
		...
		registerProperty(smbusInterface, "DiscoveredFlag", convertToString(discoveredFlag));
...
void SMBusBinding::setMuxIdleMode(const MuxIdleModes mode)
{
	...
	if (itr == muxIdleModesMap.end()) {
		phosphor::logging::log<phosphor::logging::level::ERR>("Inavlid mux idle mode");
		return;
	}
	std::string rootPort;
	...
	if (!findFiles(rootPath, matchString, i2cMuxes)) {
		phosphor::logging::log<phosphor::logging::level::DEBUG>("No mux interfaces found");
		return;
	}
...
inline void SMBusBinding::handleMuxInotifyEvent(const std::string &name)
{
	if (boost::starts_with(name, "i2c-")) {
		...
		refreshMuxTimer.async_wait([this](const boost::system::error_code &ec2) {
...
void SMBusBinding::monitorMuxChange()
{
	...
	muxMonitor->async_read_some(
		...
		[&](const boost::system::error_code &ec, std::size_t bytesTransferred) {
			if (ec) {
				phosphor::logging::log<phosphor::logging::level::ERR>(
					("monitorMuxChange: Callback Error " + ec.message()).c_str());
				return;
			}
			size_t index = 0;
...
void SMBusBinding::setupMuxMonitor()
{
	...
	int watch = inotify_add_watch(fd, "/dev", IN_CREATE | IN_MOVED_TO | IN_DELETE);
	if (watch < 0) {
...
void SMBusBinding::initializeBinding()
{
	try {
		...
		phosphor::logging::log<phosphor::logging::level::INFO>("Scanning root port");
		setMuxIdleMode(MuxIdleModes::muxIdleModeDisconnect);
		// Scan root port
		...
		auto error = "Failed to initialise SMBus binding: " + std::string(e.what());
...
std::string SMBusBinding::SMBusInit()
{
	...
	mctp_set_rx_all(mctp, &MctpBinding::rxMessage, static_cast<MctpBinding *>(this));
	mctp_set_rx_raw(mctp, &MctpBinding::onRawMessage);
	mctp_set_rx_ctrl(mctp,
					 &MctpBinding::handleMCTPControlRequests,
	...
	std::string inputDevice =
		"/sys/bus/i2c/devices/" + rootPort + "-" + hexSlaveAddr + "/slave-mqueue";
	...
	if (inFd < 0) {
		std::string newInputDevice = "/sys/bus/i2c/devices/i2c-" + rootPort + "/new_device";
...
{
	smbusReceiverFd.async_wait(boost::asio::posix::descriptor_base::wait_error, [this](auto &ec) {
		if (ec) {
			phosphor::logging::log<phosphor::logging::level::ERR>("Error: mctp_smbus_read()");
			readResponse();
		}
...
SMBusBinding::getLocationCode(const std::vector<uint8_t> &bindingPrivate)
{
	...
	auto smbusBindingPvt = reinterpret_cast<const mctp_smbus_pkt_private *>(bindingPrivate.data());
	...
	if (!std::filesystem::exists(muxSymlinkDirPath) ||
		...
		phosphor::logging::log<phosphor::logging::level::WARNING>("/dev/i2c-mux does not exist");
		return std::nullopt;
	}
	for (auto file : std::filesystem::recursive_directory_iterator(muxSymlinkDirPath)) {
		if (!file.is_symlink()) {
			continue;
		}
		std::string linkPath = std::filesystem::read_symlink(file.path()).string();
		if (boost::algorithm::ends_with(linkPath, "i2c-" + std::to_string(busNum))) {
			std::string slotName = file.path().filename().string();
			// Only take the part before "_Mux" in mux name
			std::string muxFullname = file.path().parent_path().filename().string();
			std::string muxName		= muxFullname.substr(0, muxFullname.find("_Mux"));
...
void SMBusBinding::populateDeviceProperties(const mctp_eid_t			eid,
											const std::vector<uint8_t> &bindingPrivate)
{
	auto smbusBindingPvt = reinterpret_cast<const mctp_smbus_pkt_private *>(bindingPrivate.data());

	std::string mctpEpObj = "/xyz/openbmc_project/mctp/device/" + std::to_string(eid);

	std::shared_ptr<dbus_interface> smbusIntf;
	smbusIntf = objectServer->add_interface(mctpEpObj, I2CDeviceDecorator::interface);
	smbusIntf->register_property<size_t>("Bus", getBusNumByFd(smbusBindingPvt->fd));
	smbusIntf->register_property<size_t>("Address", smbusBindingPvt->slave_addr);
...
void SMBusBinding::initEndpointDiscovery(boost::asio::yield_context &yield)
{
	...
	if (addRootDevices) {
		...
		for (const auto &device : rootDeviceMap) { registerDeviceMap.insert(device); }
	}
	...
	for (const auto &device : registerDeviceMap) {
		phosphor::logging::log<phosphor::logging::level::DEBUG>(
			("Device discovery: Checking device " + std::to_string(std::get<1>(device))).c_str());
		...
		smbusBindingPvt.slave_addr = static_cast<uint8_t>((std::get<1>(device) << 1));
		...
		std::optional<mctp_eid_t> eid = registerEndpoint(yield, bindingPvtVect, registeredEid);

		if (eid.has_value() && eid.value() != MCTP_EID_NULL) {
			DeviceTableEntry_t entry		  = std::make_pair(eid.value(), smbusBindingPvt);
			...
			auto logDeviceDetails = [&]() {
				phosphor::logging::log<phosphor::logging::level::INFO>(
					("SMBus device at bus:" + std::to_string(getBusNumByFd(smbusBindingPvt.fd)) +
					 ", 8 bit address: " + std::to_string(smbusBindingPvt.slave_addr) +
	...
	if (registerDeviceMap.empty()) {
		phosphor::logging::log<phosphor::logging::level::DEBUG>("No device found");
...
bool SMBusBinding::handleGetEndpointId(mctp_eid_t			 destEid,
									   void					*bindingPrivate,
									   std::vector<uint8_t> &request,
									   std::vector<uint8_t> &response)
{
	if (!MctpBinding::handleGetEndpointId(destEid, bindingPrivate, request, response)) {
		return false;
	}
...
bool SMBusBinding::handleSetEndpointId(mctp_eid_t			 destEid,
									   void					*bindingPrivate,
									   std::vector<uint8_t> &request,
									   std::vector<uint8_t> &response)
{
	if (!MctpBinding::handleSetEndpointId(destEid, bindingPrivate, request, response)) {
		return false;
	}
...
bool SMBusBinding::handleGetVersionSupport(mctp_eid_t			 destEid,
										   ...
										   std::vector<uint8_t> &response)
{
	if (!MctpBinding::handleGetVersionSupport(destEid, bindingPrivate, request, response)) {
		return false;
	}
...
bool SMBusBinding::handleGetMsgTypeSupport(mctp_eid_t			 destEid,
										   ...
										   std::vector<uint8_t> &response)
{
	if (!MctpBinding::handleGetMsgTypeSupport(destEid, bindingPrivate, request, response)) {
		return false;
	}
									   ...
									   std::vector<uint8_t>	 &response)
{
	...
	struct mctp_ctrl_resp_get_vdm_support *libResp =
		reinterpret_cast<struct mctp_ctrl_resp_get_vdm_support *>(response.data());
	...
	if (setIndex + 1U > vdmSetDatabase.size()) {
		...
		response.resize(sizeof(mctp_ctrl_msg_hdr) + sizeof(resp->completion_code));
		return true;
	}
...
{
	smbusDeviceTable.erase(
		std::remove_if(smbusDeviceTable.begin(),
					   smbusDeviceTable.end(),
					   [eid](auto const &tableEntry) { return (tableEntry.first == eid); }),
		smbusDeviceTable.end());
}

mctp_eid_t SMBusBinding::getEIDFromDeviceTable(const std::vector<uint8_t> &bindingPrivate)
{
	...
	for (auto &deviceEntry : smbusDeviceTable) {
		...
		if (bindingDataEntry.slave_addr == ptr->slave_addr && bindingDataEntry.fd == ptr->fd) {
...
void SMBusBinding::addUnknownEIDToDeviceTable(const mctp_eid_t eid, void *bindingPrivate)
{
	...
	auto deviceIter =
		std::find_if(smbusDeviceTable.begin(), smbusDeviceTable.end(), [eid](auto const eidEntry) {
			return std::get<0>(eidEntry) == eid;
		});
	...
	smbusBindingPvt.slave_addr = static_cast<uint8_t>((bindingPtr->slave_addr) & (~1));

	smbusDeviceTable.emplace_back(std::make_pair(eid, smbusBindingPvt));

	phosphor::logging::log<phosphor::logging::level::INFO>(
		("New EID added to device table. EID = " + std::to_string(eid)).c_str());
}

bool SMBusBinding::isBindingDataSame(const mctp_smbus_pkt_private &dataMain,
									 const mctp_smbus_pkt_private &dataTmp)
{
	if (std::tie(dataMain.fd, dataMain.slave_addr) == std::tie(dataTmp.fd, dataTmp.slave_addr)) {
		return true;
	}
	return false;
}

bool SMBusBinding::isDeviceTableChanged(const std::vector<DeviceTableEntry_t> &tableMain,
										const std::vector<DeviceTableEntry_t> &tableTmp)
{
	...
	for (size_t i = 0; i < tableMain.size(); i++) {
		if ((std::get<0>(tableMain[i]) != std::get<0>(tableTmp[i])) ||
			(!isBindingDataSame(std::get<1>(tableMain[i]), std::get<1>(tableTmp[i])))) {
			return true;
		}
	}
	return false;
...
bool SMBusBinding::isDeviceEntryPresent(const DeviceTableEntry_t			  &deviceEntry,
...
void SMBusBinding::updateRoutingTable()
{
	...
	std::vector<uint8_t> prvData =
		std::vector<uint8_t>(pktPrvPtr, pktPrvPtr + sizeof(mctp_smbus_pkt_private));
	...
	boost::asio::spawn(io, [prvData, this](boost::asio::yield_context yield) {
		...
		while ((entryHandle != 0xff) && (entryHdlCounter < 0xff)) {
			if (!getRoutingTableCtrlCmd(yield,
										prvData,
										busOwnerEid,
										entryHandle,
										getRoutingTableEntryResp)) {
				phosphor::logging::log<phosphor::logging::level::ERR>("Get Routing Table failed");
				return;
			}

			auto routingTableHdr = reinterpret_cast<mctp_ctrl_resp_get_routing_table *>(
				getRoutingTableEntryResp.data());
			size_t phyAddrOffset = sizeof(mctp_ctrl_resp_get_routing_table);

			for (uint8_t entryIndex = 0; entryIndex < routingTableHdr->number_of_entries;
				 entryIndex++) {
				...
				if ((routingTableEntry->phys_transport_binding_id == MCTP_BINDING_SMBUS) &&
					...
					smbusBindingPvt.slave_addr =
						static_cast<uint8_t>((getRoutingTableEntryResp[phyAddrOffset] << 1));

					for (uint8_t eidRange = 0; eidRange < routingTableEntry->eid_range_size;
						 eidRange++) {
						smbusDeviceTableTmp.push_back(
							std::make_pair(routingTableEntry->starting_eid + eidRange,
										   smbusBindingPvt));
					}
				}
	...
	smbusRoutingTableTimer->expires_after(std::chrono::seconds(smbusRoutingInterval));
	smbusRoutingTableTimer->async_wait(std::bind(&SMBusBinding::updateRoutingTable, this));
}

/* Function takes new routing table, detect changes and creates or removes
 * device interfaces on dbus.
 */
void SMBusBinding::processRoutingTableChanges(const std::vector<DeviceTableEntry_t> &newTable,
											  ...
											  const std::vector<uint8_t>			&prvData)
{
	...
	for (auto &deviceTableEntry : newTable) {
		if (!isDeviceEntryPresent(deviceTableEntry, smbusDeviceTable)) {
			registerEndpoint(yield,
							 prvData,
							 std::get<0>(deviceTableEntry),
							 mctp_server::BindingModeTypes::Endpoint);
		}
	}
}

void SMBusBinding::updateRoutingTableEntry(mctpd::RoutingTable::Entry  entry,
										   const std::vector<uint8_t> &privateData)
{
	...
	auto smbusData = reinterpret_cast<const mctp_smbus_pkt_private *>(privateData.data());
	entry.routeEntry.phys_address[0]				= smbusData->slave_addr;  // 8bit address
	entry.routeEntry.routing_info.phys_address_size = sizeof(smbusData->slave_addr);
