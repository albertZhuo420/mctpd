...
MCTPBridge::MCTPBridge(boost::asio::io_context &ioc, std::shared_ptr<object_server> &objServer) :
							   ...
							   std::vector<uint8_t>		  &resp)
{
	...
	if (PacketState::receivedResponse !=
		...
		phosphor::logging::log<phosphor::logging::level::ERR>("Get EID: Unable to get response");
		return false;
	}
							   ...
							   std::vector<uint8_t>			 &resp)
{
	...
	if (PacketState::receivedResponse !=
		sendAndRcvMctpCtrl(yield, req, destEid, bindingPrivate, resp)) {
		phosphor::logging::log<phosphor::logging::level::ERR>("Set EID: Unable to get response");
		return false;
	}
								...
								std::vector<uint8_t>	   &resp)
{
	...
	if (PacketState::receivedResponse !=
		sendAndRcvMctpCtrl(yield, req, destEid, bindingPrivate, resp)) {
		phosphor::logging::log<phosphor::logging::level::ERR>("Get UUID: Unable to get response");
		return false;
	}
...
bool MCTPBridge::getMsgTypeSupportCtrlCmd(boost::asio::yield_context &yield,
										  const std::vector<uint8_t> &bindingPrivate,
										  const mctp_eid_t			  destEid,
										  MsgTypeSupportCtrlResp	 *msgTypeSupportResp)
{
	...
	if (completionCode != MCTP_CTRL_CC_SUCCESS || resp.size() <= minMsgTypeRespLen) {
		phosphor::logging::log<phosphor::logging::level::ERR>(
			"Get Message Type Support: Invalid response",
	...
	std::copy_n(resp.begin(), minMsgTypeRespLen, reinterpret_cast<uint8_t *>(msgTypeSupportResp));
	...
	msgTypeSupportResp->msgType.assign(resp.begin() + minMsgTypeRespLen, resp.end());

	phosphor::logging::log<phosphor::logging::level::DEBUG>("Get Message Type Support success");
	return true;
}
	...
	MctpVersionSupportCtrlResp *mctpVersionSupportCtrlResp)
{
	...
	if (completionCode != MCTP_CTRL_CC_SUCCESS || resp.size() <= minMsgTypeRespLen) {
		...
		std::copy(respHeader.begin(),
				  respHeader.end(),
				  reinterpret_cast<uint8_t *>(&mctpVersionSupportCtrlResp->ctrlMsgHeader));
		mctpVersionSupportCtrlResp->completionCode = completionCode;
		return false;
	}

	std::copy_n(resp.begin(),
				minMsgTypeRespLen,
	...
	for (size_t iter = 1; iter <= mctpVersionSupportCtrlResp->verNoEntryCount; iter++) {
		size_t verNoEntryStartOffset		 = minMsgTypeRespLen + (mctpVersionLen * (iter - 1));
		...
		mctpVersionSupportCtrlResp->verNoEntry.push_back(std::move(versionData));
	}

	phosphor::logging::log<phosphor::logging::level::DEBUG>("Get MCTP Version Support success");
	return true;
}

bool MCTPBridge::getPCIVDMessageSupportCtrlCmd(boost::asio::yield_context &yield,
											   const std::vector<uint8_t> &bindingPrivate,
											   const mctp_eid_t			   destEid,
											   std::vector<uint16_t>	  &vendorSetIdList,
											   std::string				  &venFormatData)
{
	phosphor::logging::log<phosphor::logging::level::DEBUG>(
	...
	while (vendorIdSet < 255) {
		...
		if (!getFormattedReq<MCTP_CTRL_CMD_GET_VENDOR_MESSAGE_SUPPORT>(req, vendorIdSet)) {
			phosphor::logging::log<phosphor::logging::level::ERR>(
		...
		//     3           1          1          1             2             2
		//     (bytes)
		const ssize_t pciVDMessageSupportCmdRespSize = sizeof(MctpVendIdMsgSupportResp);
		...
		std::copy_n(resp.begin(),
					pciVDMessageSupportCmdRespSize,
		...
		vendorSetIdList.push_back(htobe16(pciVendIdMsgSupportResp.vendorIdSetCmdType));
		...
		vendorIdSet++;
		if (vendorIdSet == 255 &&
			pciVendIdMsgSupportResp.vendorIdSet != noMoreSet) {	 // invalid scenario iteration
			phosphor::logging::log<phosphor::logging::level::ERR>(
...
bool MCTPBridge::getRoutingTableCtrlCmd(boost::asio::yield_context &yield,
										const std::vector<uint8_t> &bindingPrivate,
										const mctp_eid_t			destEid,
										uint8_t						entryHandle,
										std::vector<uint8_t>	   &resp)
{
	...
	if (!getFormattedReq<MCTP_CTRL_CMD_GET_ROUTING_TABLE_ENTRIES>(req, entryHandle)) {
		phosphor::logging::log<phosphor::logging::level::ERR>(
	...
	if (!checkMinRespSize(resp)) {
		phosphor::logging::log<phosphor::logging::level::ERR>("Invalid response length");
		return false;
	}
	...
	for (uint8_t i = 0; i < routingTableHdr->number_of_entries; i++) {
		get_routing_table_entry *routingTableEntry =
			reinterpret_cast<get_routing_table_entry *>(resp.data() + entryOffset);
	...
	phosphor::logging::log<phosphor::logging::level::DEBUG>("Get Routing Table Entry success");
	return true;
}
	...
	const mctp_eid_t							eid)
{
	...
	if (find(incompatibleEIDs.begin(), incompatibleEIDs.end(), eid) != incompatibleEIDs.end()) {
		return;
	}

	auto versionIter =
		std::find_if(versionsData.begin(), versionsData.end(), [this](const auto &versionEntry) {
...
static std::optional<mctp_eid_t> checkEIDMismatchAndGetEID(mctp_eid_t eid, mctp_eid_t destEID)
{
	if (eid != destEID && eid != MCTP_EID_NULL && destEID != MCTP_EID_NULL) {
		phosphor::logging::log<phosphor::logging::level::ERR>(
			"EID mismatch detected",
...
MCTPBridge::busOwnerRegisterEndpoint(boost::asio::yield_context &yield,
									 const std::vector<uint8_t> &bindingPrivate,
									 mctp_eid_t					 eid)
{
	MctpVersionSupportCtrlResp getMctpControlVersion = {};
	if (!(getMctpVersionSupportCtrlCmd(yield,
									   bindingPrivate,
									   MCTP_EID_NULL,
									   MCTP_MESSAGE_TYPE_MCTP_CTRL,
									   &getMctpControlVersion))) {
		phosphor::logging::log<phosphor::logging::level::DEBUG>("Get MCTP Control Version failed");
		return std::nullopt;
	}
	...
	if (!(getEidCtrlCmd(yield, bindingPrivate, MCTP_EID_NULL, getEidResp))) {
		phosphor::logging::log<phosphor::logging::level::DEBUG>("Get EID failed");
		return std::nullopt;
	}
	const mctp_ctrl_resp_get_eid *getEidRespPtr =
		reinterpret_cast<mctp_ctrl_resp_get_eid *>(getEidResp.data());
	std::optional<mctp_eid_t> destEID = checkEIDMismatchAndGetEID(eid, getEidRespPtr->eid);
	...
	if (!(getUuidCtrlCmd(yield, bindingPrivate, MCTP_EID_NULL, getUuidResp))) {
		phosphor::logging::log<phosphor::logging::level::DEBUG>("Get UUID failed");
	...
	if (!(setEidCtrlCmd(yield, bindingPrivate, MCTP_EID_NULL, set_eid, eid, setEidResp))) {
		phosphor::logging::log<phosphor::logging::level::DEBUG>("Set EID failed");
	...
	if (!(getMsgTypeSupportCtrlCmd(yield, bindingPrivate, eid, &msgTypeSupportResp))) {
		phosphor::logging::log<phosphor::logging::level::DEBUG>("Get Message Type Support failed");
		return std::nullopt;
	}
...
void MCTPBridge::getVendorDefinedMessageTypes(boost::asio::yield_context  yield,
											  const std::vector<uint8_t> &bindingPrivate,
											  mctp_eid_t				  destEid,
											  EndpointProperties		 &epProperties)
{
	if (epProperties.endpointMsgTypes.vdpci) {
		...
		if (!getPCIVDMessageSupportCtrlCmd(yield,
										   bindingPrivate,
										   destEid,
										   vendorSetIdList,
										   vendorFormat)) {
			phosphor::logging::log<phosphor::logging::level::ERR>("Get Vendor Id Support failed");
			/*
		...
		epProperties.vendorIdCapabilitySets.assign(vendorSetIdList.begin(), vendorSetIdList.end());
...
formatRoutingInfoUpdateCommand(std::vector<RoutingTableEntry::MCTPLibData> &entries)
{
	std::vector<uint8_t> req;
	req.resize(sizeof(mctp_ctrl_cmd_routing_info_update) +
			   sizeof(get_routing_table_entry_with_address) * entries.size());

	auto routingInfoUpdate = reinterpret_cast<mctp_ctrl_cmd_routing_info_update *>(req.data());

	size_t formattedSize = 0;
	if (!mctp_encode_ctrl_cmd_routing_information_update(routingInfoUpdate,
														 getRqDgramInst(),
														 entries.data(),
...
void MCTPBridge::sendRoutingTableEntries(
	...
	const mctp_eid_t								   eid)
{
	boost::asio::spawn([entries = entries, eid, bindingPrivateData, this](
		...
		if (PacketState::receivedResponse !=
			sendAndRcvMctpCtrl(yield, req, eid, bindingPrivateData.value(), resp)) {
			phosphor::logging::log<phosphor::logging::level::ERR>(
				"RoutingInfoUpdate: Unable to get response");
			return;
		}

		if (!checkRespSizeAndCompletionCode<mctp_ctrl_resp_routing_info_update>(resp)) {
			phosphor::logging::log<phosphor::logging::level::ERR>(
...
void MCTPBridge::sendNewRoutingTableEntryToAllBridges(const mctpd::RoutingTable::Entry &entry)
{
	std::vector<RoutingTableEntry::MCTPLibData> libmctpEntries {entry.routeEntry};

	auto &entries = this->routingTable.getAllEntries();
	for (const auto &[eid, val] : entries) {
		// Send only to downstream bridges
		if (val.isBridge() && (eid != entry.routeEntry.routing_info.starting_eid) &&
			!val.isUpstream) {
...
void MCTPBridge::sendRoutingTableEntriesToBridge(const mctp_eid_t			 bridge,
												 const std::vector<uint8_t> &bindingPrivate)
{
