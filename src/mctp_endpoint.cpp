...
void MCTPEndpoint::handleCtrlReq(uint8_t	 destEid,
								 void		*bindingPrivate,
								 const void *req,
								 size_t		 len,
								 uint8_t	 msgTag)
{
	...
	mctp_ctrl_msg_hdr	*reqHeader = reinterpret_cast<mctp_ctrl_msg_hdr *>(request.data());
	...
	case MCTP_CTRL_CMD_PREPARE_ENDPOINT_DISCOVERY: {
		sendResponse =
			handlePrepareForEndpointDiscovery(destEid, bindingPrivate, request, response);
		break;
	}
	case MCTP_CTRL_CMD_ENDPOINT_DISCOVERY: {
		sendResponse = handleEndpointDiscovery(destEid, bindingPrivate, request, response);
		break;
	}
	case MCTP_CTRL_CMD_GET_ENDPOINT_ID: {
		sendResponse = handleGetEndpointId(destEid, bindingPrivate, request, response);
		break;
	}
	case MCTP_CTRL_CMD_SET_ENDPOINT_ID: {
		sendResponse = handleSetEndpointId(destEid, bindingPrivate, request, response);
		break;
	}
	case MCTP_CTRL_CMD_GET_VERSION_SUPPORT: {
		sendResponse = handleGetVersionSupport(destEid, bindingPrivate, request, response);
		break;
	}
	case MCTP_CTRL_CMD_GET_MESSAGE_TYPE_SUPPORT: {
		sendResponse = handleGetMsgTypeSupport(destEid, bindingPrivate, request, response);
		break;
	}
	case MCTP_CTRL_CMD_GET_VENDOR_MESSAGE_SUPPORT: {
		sendResponse = handleGetVdmSupport(destEid, bindingPrivate, request, response);
		break;
	}
	...
	default: {
		phosphor::logging::log<phosphor::logging::level::ERR>("Message not supported");
	}
	}
	...
	if (sendResponse) {
		...
		*respHeader		= *reqHeader;
		respHeader->rq_dgram_inst &= static_cast<uint8_t>(~MCTP_CTRL_HDR_FLAG_REQUEST);
		mctp_message_tx(mctp,
						destEid,
						response.data(),
						response.size(),
						false,
...
bool MCTPEndpoint::handlePrepareForEndpointDiscovery(mctp_eid_t,
													 void *,
													 std::vector<uint8_t> &,
													 std::vector<uint8_t> &)
{
...
bool MCTPEndpoint::handleEndpointDiscovery(mctp_eid_t,
										   void *,
										   std::vector<uint8_t> &,
										   std::vector<uint8_t> &)
{
...
bool MCTPEndpoint::handleGetEndpointId(mctp_eid_t destEid,
									   void *,
									   std::vector<uint8_t> &,
									   std::vector<uint8_t> &response)
{
	...
	bool busownerMode = bindingModeType == mctp_server::BindingModeTypes::BusOwner ? true : false;
...
bool MCTPEndpoint::handleGetVersionSupport(mctp_eid_t,
										   void *,
										   std::vector<uint8_t> &request,
										   std::vector<uint8_t> &response)
{
	response.resize(sizeof(mctp_ctrl_resp_get_mctp_ver_support));
	auto req  = reinterpret_cast<mctp_ctrl_cmd_get_mctp_ver_support *>(request.data());
	auto resp = reinterpret_cast<mctp_ctrl_resp_get_mctp_ver_support *>(response.data());
	...
	if (versionNumbersForUpperLayerResponder.find(req->msg_type_number) ==
		...
		resp->completion_code = MCTP_CTRL_CC_GET_MCTP_VER_SUPPORT_UNSUPPORTED_TYPE;
	}
	else {
		versions.push_back(versionNumbersForUpperLayerResponder.at(req->msg_type_number));
...
bool MCTPEndpoint::handleGetMsgTypeSupport(mctp_eid_t,
										   void *,
										   std::vector<uint8_t> &,
										   std::vector<uint8_t> &response)
{
	...
	auto resp = reinterpret_cast<mctp_ctrl_resp_get_msg_type_support *>(response.data());
	resp->completion_code = MCTP_CTRL_CC_SUCCESS;
	resp->msg_type_count  = static_cast<uint8_t>(supportedMsgTypes.size());
	std::copy(supportedMsgTypes.begin(), supportedMsgTypes.end(), std::back_inserter(response));
	return true;
}

bool MCTPEndpoint::handleGetVdmSupport([[maybe_unused]] mctp_eid_t			  destEid,
										 ...
										 std::vector<uint8_t>		&response)
{
	...
	auto getRoutingTableRequest =
		reinterpret_cast<const mctp_ctrl_cmd_get_routing_table *>(request.data());
	auto dest = reinterpret_cast<mctp_ctrl_resp_get_routing_table *>(response.data());
	...
	for (const auto &[eid, data] : entries) { entriesLibFormat.emplace_back(data.routeEntry); }
	...
	if (!mctp_encode_ctrl_cmd_rsp_get_routing_table(dest,
													entriesLibFormat.data(),
													static_cast<uint8_t>(entriesLibFormat.size()),
													&formattedRespSize)) {
...
bool MCTPEndpoint::discoveryNotifyCtrlCmd(boost::asio::yield_context &yield,
										  ...
										  const mctp_eid_t			  destEid)
{
	...
	phosphor::logging::log<phosphor::logging::level::DEBUG>("Discovery Notify success");
	return true;
}