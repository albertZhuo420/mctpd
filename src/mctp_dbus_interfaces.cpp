...
MCTPDBusInterfaces::MCTPDBusInterfaces(std::shared_ptr<object_server> &objServer) :
  objectServer(objServer)
{ }

MCTPDBusInterfaces::~MCTPDBusInterfaces()
{
	for (auto &iter : endpointInterface) { objectServer->remove_interface(iter.second); }

	for (auto &iter : msgTypeInterface) { objectServer->remove_interface(iter.second); }

	for (auto &iter : uuidInterface) { objectServer->remove_interface(iter.second); }

	for (auto &iter : vendorIdInterface) { objectServer->remove_interface(iter.second); }
	for (auto &iter : locationCodeInterface) { objectServer->remove_interface(iter.second); }
	for (auto &iter : deviceInterface) { objectServer->remove_interface(iter.second); }

	objectServer->remove_interface(mctpInterface);
}

void MCTPDBusInterfaces::populateDeviceProperties(const mctp_eid_t, const std::vector<uint8_t> &)
{
	// Do nothing
}

bool MCTPDBusInterfaces::removeInterface(mctp_eid_t eid, endpointInterfaceMap &interfaces)
{
...
void MCTPDBusInterfaces::registerMsgTypes(std::shared_ptr<dbus_interface> &msgTypeIntf,
...
void MCTPDBusInterfaces::populateEndpointProperties(const EndpointProperties &epProperties)
{
	std::string						mctpDevObj = "/xyz/openbmc_project/mctp/device/";
	std::shared_ptr<dbus_interface> endpointIntf;
	std::string mctpEpObj = mctpDevObj + std::to_string(epProperties.endpointEid);

	// Endpoint interface
	endpointIntf = objectServer->add_interface(mctpEpObj, mctp_endpoint::interface);
	...
	endpointInterface.emplace(epProperties.endpointEid, std::move(endpointIntf));

	// UUID interface
	std::shared_ptr<dbus_interface> uuidIntf;
	uuidIntf = objectServer->add_interface(mctpEpObj, "xyz.openbmc_project.Common.UUID");
	...
	if (epProperties.endpointMsgTypes.vdpci) {
		...
		vendorIdIntf =
			objectServer->add_interface(mctpEpObj, "xyz.openbmc_project.MCTP.PCIVendorDefined");
		vendorIdIntf->register_property("MessageTypeProperty",
										epProperties.vendorIdCapabilitySets);
		vendorIdIntf->register_property("VendorID", epProperties.vendorIdFormat);
		vendorIdIntf->initialize();
		vendorIdInterface.emplace(epProperties.endpointEid, std::move(vendorIdIntf));
	}

	// Location code interface
	std::shared_ptr<dbus_interface> locationCodeIntf;
	locationCodeIntf = objectServer->add_interface(mctpEpObj, LocationCodeDecorator::interface);
	locationCodeIntf->register_property("LocationCode", epProperties.locationCode);
	locationCodeIntf->initialize();
	locationCodeInterface.emplace(epProperties.endpointEid, std::move(locationCodeIntf));

	// Message type interface
	// This interface should be added last as adding it will trigger mctpwplus
	// deviceAdded event
	std::shared_ptr<dbus_interface> msgTypeIntf;
	msgTypeIntf = objectServer->add_interface(mctpEpObj, mctp_msg_types::interface);
