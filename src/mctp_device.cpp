...
MCTPDevice::MCTPDevice(boost::asio::io_context &ioc, std::shared_ptr<object_server> &objServer) :
  MCTPDBusInterfaces(objServer), io(ioc), ctrlTxTimer(ioc)
{
	initializeLogging();
	mctp = mctp_init();
	if (!mctp) {
		phosphor::logging::log<phosphor::logging::level::ERR>("Failed to init mctp");
...
std::optional<std::vector<uint8_t>> MCTPDevice::getBindingPrivateData(uint8_t /*dstEid*/)
{
	// No Binding data by default
	return std::vector<uint8_t>();
}

std::optional<std::string> MCTPDevice::getLocationCode(const std::vector<uint8_t> &)
{
	return std::nullopt;
}

void MCTPDevice::updateRoutingTableEntry(mctpd::RoutingTable::Entry, const std::vector<uint8_t> &)
{
	// Do nothing
}
...
bool MCTPDevice::handleCtrlResp(void *msg, const size_t len)
{
	...
	auto reqItr = std::find_if(ctrlTxQueue.begin(), ctrlTxQueue.end(), [&](auto &ctrlTx) {
		auto &[state, retryCount, maxRespDelay, destEid, bindingPrivate, req, callback] = ctrlTx;

		mctp_ctrl_msg_hdr *reqHeader = reinterpret_cast<mctp_ctrl_msg_hdr *>(req.data());
		...
		if (getInstanceId(reqHeader->rq_dgram_inst) == getInstanceId(respHeader->rq_dgram_inst)) {
			phosphor::logging::log<phosphor::logging::level::DEBUG>(
				"Matching Control command request found");

			uint8_t				*tmp  = reinterpret_cast<uint8_t *>(msg);
			std::vector<uint8_t> resp = std::vector<uint8_t>(tmp, tmp + len);
...
void MCTPDevice::processCtrlTxQueue()
{
	...
	ctrlTxTimer.async_wait([this](const boost::system::error_code &ec) {
		if (ec == boost::asio::error::operation_aborted) {
			...
			phosphor::logging::log<phosphor::logging::level::ERR>("ctrlTxTimer failed");
			return;
		}
		...
		ctrlTxQueue.erase(
			std::remove_if(
				...
				[this](auto &ctrlTx) {
					...
					if (maxRespDelay > 0 && state != PacketState::receivedResponse) {
						if (retryCount > 0 && maxRespDelay <= retryCount * ctrlTxRetryDelay) {
...
{
	ctrlTxQueue.push_back(std::make_tuple(state,
										  ctrlTxRetryCount,
										  ((ctrlTxRetryCount + 1) * ctrlTxRetryDelay),
										  destEid,
										  bindingPrivate,
										  req,
										  callback));
	...
	if (sendMctpCtrlMessage(destEid, req, true, 0, bindingPrivate)) {
		phosphor::logging::log<phosphor::logging::level::DEBUG>("Packet transmited");
		state = PacketState::transmitted;
	}
...
PacketState MCTPDevice::sendAndRcvMctpCtrl(boost::asio::yield_context &yield,
										   const std::vector<uint8_t> &req,
										   const mctp_eid_t			   destEid,
										   const std::vector<uint8_t> &bindingPrivate,
										   std::vector<uint8_t>		  &resp)
{
	...
	std::function<void(PacketState, std::vector<uint8_t> &)> callback =
		[&resp, &pktState, &timer](PacketState state, std::vector<uint8_t> &response) {
			phosphor::logging::log<phosphor::logging::level::DEBUG>("Callback triggered");
			...
			phosphor::logging::log<phosphor::logging::level::DEBUG>(
				("Packet state: " + std::to_string(static_cast<int>(pktState))).c_str());
		};
...
mctp_server::BindingModeTypes MCTPDevice::getEndpointType(const uint8_t types)
{
	...
	if (endpointType == simpleEndpoint) {
		...
		phosphor::logging::log<phosphor::logging::level::ERR>("Invalid endpoint type value");
		throw;
	}
}
...
MsgTypes MCTPDevice::getMsgTypes(const std::vector<uint8_t> &msgType)
{
	...
	for (auto type : msgType) {
		...
		default: {
			...
			phosphor::logging::log<phosphor::logging::level::ERR>("Invalid message type");
			break;
		}
		}
	}
...
bool MCTPDevice::isEIDMappedToUUID(const mctp_eid_t eid, const std::string &destUUID)
{
	...
	if (eidFromTable.has_value()) {
		...
		if (eid == eidFromTable.value()) {
			phosphor::logging::log<phosphor::logging::level::DEBUG>(
				("Endpoint already Registered with EID " + std::to_string(eid)).c_str());
			return true;
		}
		phosphor::logging::log<phosphor::logging::level::INFO>(
			("Endpoint needs re-registration. EID from device:" + std::to_string(eid) +
...
std::optional<mctp_eid_t> MCTPDevice::getEIDForReregistration(const std::string &destUUID)
{
