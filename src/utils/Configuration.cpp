...
using ConfigurationField =
	std::variant<bool, uint64_t, std::string, std::vector<uint64_t>, std::vector<std::string>>;

using ConfigurationMap = std::unordered_map<std::string, ConfigurationField>;

static const std::string mctpTypeName = "xyz.openbmc_project.Configuration.MctpConfiguration";

static const std::string boardPathNamespace = "/xyz/openbmc_project/inventory/system/board";

static const std::unordered_map<std::string, mctp_server::BindingModeTypes>
	stringToBindingModeMap = {{"busowner", mctp_server::BindingModeTypes::BusOwner},
...
static const std::unordered_map<std::string, mctp_server::MctpPhysicalMediumIdentifiers>
	stringToMediumID = {
		{"Smbus", mctp_server::MctpPhysicalMediumIdentifiers::Smbus},
		{"SmbusI2c", mctp_server::MctpPhysicalMediumIdentifiers::SmbusI2c},
		{"I2cCompatible", mctp_server::MctpPhysicalMediumIdentifiers::I2cCompatible},
		{"Smbus3OrI2c400khzCompatible",
		 mctp_server::MctpPhysicalMediumIdentifiers::Smbus3OrI2c400khzCompatible},
		{"Smbus3OrI2c1MhzCompatible",
		 mctp_server::MctpPhysicalMediumIdentifiers::Smbus3OrI2c1MhzCompatible},
		{"I2c3Mhz4Compatible", mctp_server::MctpPhysicalMediumIdentifiers::I2c3Mhz4Compatible},
		...
		{"PciCompatible", mctp_server::MctpPhysicalMediumIdentifiers::PciCompatible},
		{"Usb11Compatible", mctp_server::MctpPhysicalMediumIdentifiers::Usb11Compatible},
		{"Usb20Compatible", mctp_server::MctpPhysicalMediumIdentifiers::Usb20Compatible},
		{"Usb30Compatible", mctp_server::MctpPhysicalMediumIdentifiers::Usb30Compatible},
		{"NcSiOverRbt", mctp_server::MctpPhysicalMediumIdentifiers::NcSiOverRbt},
		{"KcsLegacy", mctp_server::MctpPhysicalMediumIdentifiers::KcsLegacy},
		{"KcsPci", mctp_server::MctpPhysicalMediumIdentifiers::KcsPci},
		{"SerialHostLegacy", mctp_server::MctpPhysicalMediumIdentifiers::SerialHostLegacy},
		{"SerialHostPci", mctp_server::MctpPhysicalMediumIdentifiers::SerialHostPci},
		{"AsynchronousSerial", mctp_server::MctpPhysicalMediumIdentifiers::AsynchronousSerial},
		{"I3cSDR", mctp_server::MctpPhysicalMediumIdentifiers::I3cSDR},
		{"I3cHDRDDR", mctp_server::MctpPhysicalMediumIdentifiers::I3cHDRDDR}};

template <typename T>
static bool getField(const ConfigurationMap &configuration, const std::string &fieldName, T &value)
{
...
template <typename T>
static bool getField(const json &configuration, const std::string &fieldName, T &value)
{
	...
	try {
		...
		phosphor::logging::log<phosphor::logging::level::WARNING>(
			("Error reading configuration field " + fieldName + ": " + e.what()).c_str());
		return false;
	}
}
...
std::set<std::string> getAllowedBuses(const T &map)
{
	...
	phosphor::logging::log<phosphor::logging::level::WARNING>(
		(std::string("Allowed buses in config : ") + std::to_string(allowedBuses.size())).c_str());
...
static std::optional<SMBusConfiguration> getSMBusConfiguration(const T &map)
{
	...
	if (!getField(map, "DefaultEID", defaultEID) && !getField(map, "default-eid", defaultEID)) {
		return std::nullopt;
	}
	...
	if (mode == mctp_server::BindingModeTypes::BusOwner && !getField(map, "EIDPool", eidPool) &&
	...
	if (!getField(map, "SupportedEndpointSlaveAddress", supportedEndpointSlaveAddress)) {
	...
	if (!getField(map, "IgnoredEndpointSlaveAddress", ignoredEndpintSlaveAddress)) {
		ignoredEndpintSlaveAddress = {};
	}

	auto endpointSlaveAddress = std::set<uint8_t>(supportedEndpointSlaveAddress.begin(),
...
static ConfigurationMap getConfigurationMap(std::shared_ptr<sdbusplus::asio::connection> conn,
											const std::string &configurationPath)
{
	auto method_call = conn->new_method_call("xyz.openbmc_project.EntityManager",
...
getConfigurationFromEntityManager(std::shared_ptr<sdbusplus::asio::connection> conn,
								  const std::string							  &configurationName)
{
	...
	if (bindingType == "MctpSMBus") {
		if (auto optConfig = getSMBusConfiguration(map)) {
			configuration = std::make_unique<SMBusConfiguration>(std::move(*optConfig));
		}
	}
	else if (bindingType == "MctpPCIe") {
		if (auto optConfig = getPcieConfiguration(map)) {
			configuration = std::make_unique<PcieConfiguration>(std::move(*optConfig));
		}
	}
	if (!configuration) {
		return std::nullopt;
	...
	std::regex_replace(name.begin(), name.begin(), name.end(), illegal_name_regex, "_");
						 ...
						 const std::string			 &configurationName)
{
	...
	return std::make_pair("MCTP-" + configurationName, std::move(configuration));
}
...
{
	auto configurationPair = getConfigurationFromEntityManager(conn, configurationName);
	if (!configurationPair) {
		configurationPair = getConfigurationFromFile(configPath, configurationName);
	}
