...
void DeviceWatcher::deviceDiscoveryInit()
{
	...
	for (auto itr = successiveInitCount.begin(); itr != successiveInitCount.end();) {
...
bool DeviceWatcher::checkDeviceInitThreshold(const BindingPrivateVect &bindingPvt)
{
	...
	if (search != successiveInitCount.end()) {
		...
		if (search->second == successiveDeviceInitThold) {
			...
			for (auto re : bindingPvt) {
				bindingPvtStr << " 0x" << std::hex << std::setfill('0') << std::setw(2)
							  << static_cast<int>(re);
			}
