...
constexpr sd_id128_t mctpdAppId =
	SD_ID128_MAKE(c4, e4, d9, 4a, 88, 43, 4d, f0, 94, 9d, bb, 0a, af, 53, 4e, 6d);

/* According DSP0239(Version: 1.7.0) */
static const std::unordered_map<uint8_t, mctp_server::MctpPhysicalMediumIdentifiers>
	valueToMediumId = {
		/*0x00 Unspecified*/
		{0x01, mctp_server::MctpPhysicalMediumIdentifiers::Smbus},	  /*SMBus 2.0 100 kHz
																		 compatible*/
		{0x02, mctp_server::MctpPhysicalMediumIdentifiers::SmbusI2c}, /*SMBus 2.0 + I2C 100 kHz
																		 compatible*/
		{0x03, mctp_server::MctpPhysicalMediumIdentifiers::I2cCompatible}, /*I2C 100 kHz compatible
																			  (Standard-mode)*/
		{0x04,
		...
		{0x06,
		 mctp_server::MctpPhysicalMediumIdentifiers::I2c3Mhz4Compatible}, /*I2C 3.4 MHz compatible
																			 (High-speed mode)*/
		/*0x07 Reserved*/
		{0x08,
		 mctp_server::MctpPhysicalMediumIdentifiers::Pcie11},	   /*PCIe revision 1.1 compatible*/
		{0x09, mctp_server::MctpPhysicalMediumIdentifiers::Pcie2}, /*PCIe revision 2.0
																	  compatible*/
		{0x0A,
		 mctp_server::MctpPhysicalMediumIdentifiers::Pcie21},	   /*PCIe revision 2.1 compatible*/
		{0x0B, mctp_server::MctpPhysicalMediumIdentifiers::Pcie3}, /*PCIe revision 3.x
																	  compatible*/
		{0x0C, mctp_server::MctpPhysicalMediumIdentifiers::Pcie4}, /*PCIe revision 4.x
																	  compatible*/
		{0x0D, mctp_server::MctpPhysicalMediumIdentifiers::Pcie5}, /*PCIe revision 5.x
																	  compatible*/
		/*0x0E Reserved*/
		{0x0F, mctp_server::MctpPhysicalMediumIdentifiers::PciCompatible},	 /*PCI compatible
																				(PCI 1.0,2.0,2.1,2.2,2.3,3.0,PCI-X
																				1.0,   PCI-X 2.0)*/
		{0x10, mctp_server::MctpPhysicalMediumIdentifiers::Usb11Compatible}, /*USB 1.1 compatible*/
		{0x11, mctp_server::MctpPhysicalMediumIdentifiers::Usb20Compatible}, /*USB 2.0 compatible*/
		{0x12, mctp_server::MctpPhysicalMediumIdentifiers::Usb30Compatible}, /*USB 3.0 compatible*/
		/*0x13:0x17 Reserved*/
		{0x18,
		 mctp_server::MctpPhysicalMediumIdentifiers::NcSiOverRbt}, /*NC-SI over RBT (A physical
																	  interface based on RMII as
																	  defined inDSP0222)*/
		/*0x19:0x1F Reserved*/
		{0x20, mctp_server::MctpPhysicalMediumIdentifiers::KcsLegacy}, /*KCS / Legacy (Fixed
																		  Address Decoding)*/
		{0x21, mctp_server::MctpPhysicalMediumIdentifiers::KcsPci},	   /*KCS / PCI (Base Class 0xC0
																		  Subclass 0x01)*/
		{0x22,
		 mctp_server::MctpPhysicalMediumIdentifiers::SerialHostLegacy}, /*Serial Host / Legacy
																		   (Fixed Address
																		   Decoding)*/
		{0x23,
		 mctp_server::MctpPhysicalMediumIdentifiers::SerialHostPci}, /*Serial Host / PCI (Base
																		Class 0x07 Subclass 0x00)*/
		{0x24, mctp_server::MctpPhysicalMediumIdentifiers::AsynchronousSerial}, /*Asynchronous
																				   Serial3(Between
																				   MCs and IMDs)*/
		{0x30,
		 mctp_server::MctpPhysicalMediumIdentifiers::I3cSDR}, /*I3C 12.5 MHz compatible (SDR)*/
		{0x31,
		 mctp_server::MctpPhysicalMediumIdentifiers::I3cHDRDDR} /*I3C 25 MHz compatible (HDR-DDR)*/
																/*0x32:0x3F Reserved */
																/*0x40, CXL 1.x*/
																/*0x41:0xFF Reserved*/
};
  ...
  connection(conn), mctpServiceScanner(connection), bindingID(bindingType)
{
	...
	mctpServiceScanner.setAllowedBuses(conf.allowedBuses.begin(), conf.allowedBuses.end());
	...
	mctpServiceScanner.setCallback(
		[this](bridging::MCTPServiceScanner::EndPoint ep, bool isHotplugged) {
			...
			phosphor::logging::log<phosphor::logging::level::INFO>(
				("NewEID " + std::to_string(ep.eid) + " of type " + ep.endpointType +
				 (isHotplugged ? " hotplugged" : " existing") + " on " + ep.service.name)
					.c_str());

			// TODO. Get physical medium specific details
			auto					   entryType = mctpd::convertToEndpointType(ep.endpointType);
			mctpd::RoutingTable::Entry entry(ep.eid, ep.service.name, entryType);
			entry.isUpstream = true;
			routingTable.updateEntry(ep.eid, entry);
			sendNewRoutingTableEntryToAllBridges(entry);
		});
	mctpServiceScanner.setEidRemovedCallback([this](bridging::MCTPServiceScanner::EndPoint ep) {
		if (this->routingTable.removeEntry(ep.eid)) {
			phosphor::logging::log<phosphor::logging::level::INFO>(
				(std::to_string(ep.eid) + " removed from routing table").c_str());
		}
		else {
			phosphor::logging::log<phosphor::logging::level::INFO>(
				(std::to_string(ep.eid) + " was not in routing table").c_str());
		}
	});
	...
	versionNumbersForUpperLayerResponder.insert(
		std::pair<uint8_t, version_entry> {MCTP_MESSAGE_TYPE_MCTP_CTRL, {0xF1, 0xF3, 0xF1, 0}});
	versionNumbersForUpperLayerResponder.insert(
	...
	try {
		...
		registerProperty(mctpInterface,
						 "BindingID",
		...
		registerProperty(
			mctpInterface,
			"BindingMediumID",
			mctp_server::convertMctpPhysicalMediumIdentifiersToString(bindingMediumID));
		...
		if (bindingModeType == mctp_server::BindingModeTypes::BusOwner) {
			...
			entry.routeEntry.routing_info.phys_media_type_id =
				static_cast<uint8_t>(mctpd::convertToPhysicalMediumIdentifier(bindingMediumID));
		...
		mctpInterface->register_method(
			...
			[this](uint8_t dstEid, uint8_t msgTag, bool tagOwner, std::vector<uint8_t> payload) {
				if (payload.size() > 0) {
		...
		mctpInterface->register_method(
			...
			[this](const mctp_eid_t eid, const uint16_t timeout) {
				if (!reserveBandwidth(eid, timeout)) {
					phosphor::logging::log<phosphor::logging::level::WARNING>(
						("Reserve bandwidth failed for EID: " + std::to_string(eid)).c_str());
					return static_cast<int>(mctpErrorRsvBWFailed);
				}
				phosphor::logging::log<phosphor::logging::level::DEBUG>(
					("Reserve bandwidth is active for EID: " + std::to_string(eid)).c_str());
				return static_cast<int>(mctpSuccess);
			});
		mctpInterface->register_method("ReleaseBandwidth", [this](const mctp_eid_t eid) {
			if (!releaseBandwidth(eid)) {
				phosphor::logging::log<phosphor::logging::level::ERR>(
					("Release bandwidth failed for EID: " + std::to_string(eid)).c_str());
		...
		mctpInterface->register_method(
			...
			[this](boost::asio::yield_context yield,
				...
				auto					  message = transmissionQueue.transmit(mctp,
														   dstEid,
														   std::move(payload),
														   std::move(pvtData).value(),
														   io);
		...
		mctpInterface->register_signal<uint8_t, uint8_t, uint8_t, bool, std::vector<uint8_t>>(
			"MessageReceivedSignal");

		mctpInterface->register_method(
			"RegisterResponder",
			[this](uint8_t msgTypeName, std::vector<uint8_t> inputVersion) -> bool {
				return registerUpperLayerResponder(msgTypeName, inputVersion);
			});

		// register VDPCI responder with MCTP for upper layers
		mctpInterface->register_method("RegisterVdpciResponder",
		...
		mctpInterface->register_method("SendMctpRawPayload",
 ...
 */
void MctpBinding::rxMessage(uint8_t srcEid,
							void   *data,
							void   *msg,
							size_t	len,
							bool	tagOwner,
							...
							void   *bindingPrivate)
{
	...
	if (data == nullptr) {
		phosphor::logging::log<phosphor::logging::level::ERR>("Null data received");
		return;
	}
	...
	if (!tagOwner && mctp_is_mctp_ctrl_msg(msg, len) && !mctp_ctrl_msg_is_req(msg, len)) {
		phosphor::logging::log<phosphor::logging::level::DEBUG>(
	...
	if (!tagOwner && binding.transmissionQueue
						 .receive(binding.mctp, srcEid, msgTag, std::move(response), binding.io)) {
		return;
	}
...
void MctpBinding::handleMCTPControlRequests(uint8_t srcEid,
											void   *data,
											void   *msg,
											size_t	len,
											bool	tagOwner,
...
bool MctpBinding::reserveBandwidth(const mctp_eid_t /*eid*/, const uint16_t /*timeout*/)
{
	return true;
}
...
bool MctpBinding::registerUpperLayerResponder(uint8_t typeNo, std::vector<uint8_t> &versionData)
{
	bool ret = false;
	switch (typeNo) {
	...
	case MCTP_MESSAGE_TYPE_SPDM: ret = manageVersionInfo(typeNo, versionData); break;
	default:
		phosphor::logging::log<phosphor::logging::level::ERR>(
...
bool MctpBinding::manageVersionInfo(uint8_t typeNo, std::vector<uint8_t> &versionInfo)
{
	...
	phosphor::logging::log<phosphor::logging::level::DEBUG>("Existing Data In Map for the typeNo");
	return false;
}

bool MctpBinding::manageVdpciVersionInfo(uint16_t vendorIdx, uint16_t cmdSetType)
{
	struct InternalVdmSetDatabase vdmSupport;

	auto retIter = std::find_if(
		vdmSetDatabase.begin(),
		vdmSetDatabase.end(),
		[vendorIdx](const InternalVdmSetDatabase &vm) { return vm.vendorId == vendorIdx; });
	...
	if (retIter == vdmSetDatabase.end()) {
		...
		if (retIter->commandSetType != cmdSetType) { /*may be the details need to be updated*/
...
bool MctpBinding::setMediumId(uint8_t value, mctp_server::MctpPhysicalMediumIdentifiers &mediumId)
{
...
std::optional<mctp_eid_t> MctpBinding::registerEndpoint(boost::asio::yield_context &yield,
														const std::vector<uint8_t> &bindingPrivate,
														mctp_eid_t					eid,
														mctp_server::BindingModeTypes bindingMode)
{
	if (bindingModeType == mctp_server::BindingModeTypes::BusOwner) {
		std::optional<mctp_eid_t> destEID = busOwnerRegisterEndpoint(yield, bindingPrivate, eid);
	...
	if (!(getMsgTypeSupportCtrlCmd(yield, bindingPrivate, eid, &msgTypeSupportResp))) {
		phosphor::logging::log<phosphor::logging::level::ERR>("Get Message Type Support failed");
		return std::nullopt;
	}
	...
	if (!(getUuidCtrlCmd(yield, bindingPrivate, eid, getUuidResp))) {
		...
		phosphor::logging::log<phosphor::logging::level::ERR>("Get UUID failed");
	...
	try {
		...
		if (!entry.isUpstream) {
			if (rsvBWActive && dstEid != reservedEID) {
				...
				throw std::runtime_error((("Send is not allowed. ReserveBandwidth is active "
										   "for ") +
										  std::to_string(reservedEID))
											 .c_str());
			}
			...
			if (!pvtData) {
				...
				throw std::runtime_error("SendMctpRawPayload: Invalid destination EID");
			}

			if (mctp_message_raw_tx(mctp, payload.data(), payload.size(), pvtData->data()) < 0) {
				phosphor::logging::log<phosphor::logging::level::ERR>(
			...
			auto sendCB = [dstEid](boost::system::error_code ec, int sendStatus) {
				if (ec || sendStatus != 0) {
					phosphor::logging::log<phosphor::logging::level::ERR>(
			...
			connection->async_method_call(sendCB,
										  entry.serviceName,
...
void MctpBinding::onRawMessage(void *data, void *msg, size_t len, void * /*msgBindingPrivate*/)
{
